...
//In this we choose the problem and difficulty for creating a 1-on-1 session
export async function createSession(req, res) {
  try {
    //Here we are getting the data from the client(frontend) req
    ...
    if (!problem || !difficulty)
      return res
        .status(400)
        .json({ message: "Problem and Difficulty required" });
...
export async function getActiveSessions(_, res) {
  try {
    const sessions = await Session.find({ status: "active" })
      .populate("host", "name profileImage email clerkId") //replaces host objectid with other values
